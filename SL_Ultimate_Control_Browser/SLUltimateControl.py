...
from SLBrowserControl import SLBrowserControl
...
class SLUltimateControl(ControlSurface):
    ...
    def _setup_mixer_control(self):
        is_momentary = True
        self._mixer = SLMixer(8, self._display, self._session,self)
    ...
    def _setup_custom_controls(self):
        ...
        if BROWSER_ENABLE:
            ...
            for track in range(8):
                pad_buttons.append(ButtonElement(is_momentary, MIDI_CC_TYPE, MIDI_CHANNEL, pad_row_ccs[track]))
            self._browser.set_action_controls(tuple(pad_buttons))    
        ...
        for index in range(8):
            ...
            pots.append(pot)
        self._pot_modes = SLPotModeSelector(self._mixer, tuple(pots),self)
        ...
        if BROWSER_ENABLE:
            self._shift_mode = SLShiftableSelector(tuple(select_buttons), show_pot_values_button, tuple(p1_buttons), self._scene_up_button, self._scene_down_button, self._scene_launch_button, self._session, self._mixer, self._encoder_modes, self._device, self._device_control, tuple(self._clip_launch_buttons), self._pot_modes, tuple(pad_buttons),tuple(pots) ,self)
        else:
