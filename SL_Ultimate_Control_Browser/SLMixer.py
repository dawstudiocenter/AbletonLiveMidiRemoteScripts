...
from config import INCLUDE_RETURN_TRACKS, SMART_VIEW, TRACK_SELECT_ON_ARM, INVERT_MUTE_FEEDBACK, TEMPO_TOP, TEMPO_BOTTOM, BROWSER_ENABLE
...
class SLMixer(MixerComponent):
    ...
    def __init__(self, num_tracks, display, session,parent=None):
    ...
    def show_pot_values(self):
        if BROWSER_ENABLE and self._pot_mode_index ==13: # BROWSER DEVICE MODE
            #self.debug('show pot value')
            self._browser_device.show_device_values(True)
        elif BROWSER_ENABLE and self._pot_mode_index ==12: # BROWSER DEVICE MODE
            #self.debug('show pot value')
            self._browser_device.show_device_values(True)
        elif BROWSER_ENABLE and self._pot_mode_index ==11: # BROWSER DEVICE MODE
            #self.debug('show pot value')
            self._browser_device.show_device_values(True)
        elif BROWSER_ENABLE and self._pot_mode_index ==10: # BROWSER DEVICE MODE
            #self.debug('show pot value')
            self._browser_device.show_device_values(True)
        elif BROWSER_ENABLE and self._pot_mode_index ==9: # BROWSER DEVICE MODE
            #self.debug('show pot value')
            self._browser_device.show_device_values(True)
            
        elif self._pot_mode_index == 8: # EXTRA DEVICE MODE
                ...
                if ((value != 0) or (not self._select_button.is_momentary())):
                    ...
                    if self._select_button_double_press > 0:
                        if self._track.arm == True:
                            if self.song().session_record == True:
                                self.song().session_record = False
                            else:
                                if (self._track.playing_slot_index == self._session._scene_offset or self._track.fired_slot_index == self._session._scene_offset):
